...
class RVLatestReport (private val listener:(position:Int)-> Unit):BaseListAdapter<RecyclerView.ViewHolder, HomeDataModel.ResponseData>(){
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (dataList[position].status) {
            ...
            2 -> {
                ...
                (holder).mStatus.background = context.resources.getDrawable(R.drawable.button_gradient)
            }
        }
        }
...
class ViewHolder(view:View, onClickListener:(position: Int)->Unit):RecyclerView.ViewHolder(view){
...
//        itemView.tv_penghargaan_isi.text = penghargaanModel.deskripsi
    }
}